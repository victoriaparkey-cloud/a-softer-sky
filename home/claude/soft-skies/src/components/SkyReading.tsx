...
export function SkyReading({ entry, onSave, onNewSky, saved = false }: SkyReadingProps) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      ...
      link.download = `${slug}-soft-skies.jpg`
  ...
  return (
    <div className="page-enter max-w-2xl mx-auto px-7 pb-20 pt-8">
      ...
      <div className="grid grid-cols-2 gap-x-5 gap-y-4 mt-6">
        ...
        <div className="flex flex-col gap-1">
          <label className="label-caps text-[10px]">
            Swatches ({entry.palette.length} found)
          </label>
          <input
            type="number"
            min={3}
      ...
      <div className="flex gap-3 flex-wrap mt-6">
        ...
        {saved ? (
          <span className="label-caps text-muted/60 py-3">Saved to journal</span>
        ) : (
